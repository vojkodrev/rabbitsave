...
&ast; Write to queue app waits for 3000 (configurable) lines to arrive or 250ms (also configurable). Splits them into packages (json format) by matchId % num of queues. Then sends them to queues. This is because if you send lines 1 by 1 to queues, message loss will occur in the RabbitMQ.
